...
public class TransformEffects : MonoBehaviour
{
    ...
    private void StartIdleEffect()
    {
        ...
        sequence.Append(_rectTransform.DOLocalRotate(new Vector3(20, _tiltAmplitude * 2, _originRotation.z + (_tiltAmplitude * _originRotation.z < 0 ? -1 : 1)), _tiltDuration).SetEase(Ease.OutSine));
        sequence.Append(_rectTransform.DOLocalRotate(new Vector3(-20, -_tiltAmplitude * 2, _originRotation.z + (_tiltAmplitude * _originRotation.z < 0 ? -1 : 1)), _tiltDuration).SetEase(Ease.OutSine))
